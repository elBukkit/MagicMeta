    ...
    CodeMirror.registerHelper('hint', 'yaml', function(cm, opts) {
        ...
        if (LEAF_KV.test(curLine)) {
            ...
            if (hierarchy.length == 2) {
                ...
                if (metadata.context.modifier_properties.hasOwnProperty(fieldName)) {
                    var propertyKey = metadata.context.modifier_properties[fieldName];
            ...
            if (hierarchy.length == 2 && hierarchy[1] == '') {
                // Add base parameters
                properties = metadata.context.modifier_properties;
